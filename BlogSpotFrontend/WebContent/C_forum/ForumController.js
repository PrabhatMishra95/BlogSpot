myApp.controller("ForumController", function($scope, $http, $location,$rootScope, $window) {
	...
	$scope.insertForum = function() {
		...
		$http.post("http://localhost:8078/BlogSpotMiddleware/addForum",
	...
	function fetchAllForums() {
		...
		$http.get("http://localhost:8078/BlogSpotMiddleware/listForums").then(
				function(response) {
					console.log('Status text:' + response.statusText);
					$scope.forumData = response.data;
	...
	$scope.editForum = function(forumId) {
		...
		$http.get('http://localhost:8078/BlogSpotMiddleware/getForum/' + forumId)
	...
	$scope.updateForum = function(forumId){
		...
		console.log(forumId);
		$http.put('http://localhost:8078/BlogSpotMiddleware/updateForum/'+ forumId, $scope.forum)
	...
	$scope.deleteForum = function(forumId){
		...
		$http.delete('http://localhost:8078/BlogSpotMiddleware/deleteForum/'+forumId)
		.then(fetchAllForums(), function(response){
	...
	$scope.approveForum = function(forumId){
		...
		console.log(forumId);
		$http.put('http://localhost:8078/BlogSpotMiddleware/approveForum/'+ forumId)
		.then(fetchAllForums(), function(response){
	...
	$scope.rejectForum = function(forumId){
		...
		console.log(forumId);
		$http.put('http://localhost:8078/BlogSpotMiddleware/rejectForum/'+ forumId)
		.then(fetchAllForums(), function(response){
	...
	$scope.fetchAllForumComments=function(forumId) {
		...
		$http.get("http://localhost:8078/BlogSpotMiddleware/listForumComments/"+forumId).then(
				function(response) {
					console.log('Status text:' + response.statusText);
	...
	$scope.addForumComment = function(forumId) {
		...
		$http.post("http://localhost:8078/BlogSpotMiddleware/addForumComment",
